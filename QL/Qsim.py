...
QTable = np.load('qtable_light_0911.npy')
Qcount = np.load('qcount_light_0911.npy')
#Qcount[Qcount > 3] = 3
...
def gatherStateInfo(cars, trajectories, carID):
   ...
   # carUpdate['Collision'] = car['status'] >= 2
    ...
    if (grade=='i' and car['lanepos']>5.) or grade=='o':
        ...
        if car['lanepos'] + car['speed'] > 87.: # added 5/25, temp fix
            ...
            searchAdjacentAhead = searchAdjacentAhead & (
                            cars['lanepos'] < car['lanepos']+car['speed']-92.)
    ...
    fastTNV = 100
    DTAC = 100
   # find closest vehicle for this particular crossing
    ...
    carUpdate['Priority'] = car['priority']
    return carUpdate
...
while iteration <= numiter and err == 0:    
    ...
    while ttime < maxTime and err == 0 and np.any(cars['status']<=0):
        ...
        if priorityRoad1_2:
            print "roads 1,2"
        elif priorityRoad3_4:
            print "print roads 3,4"
        else:
            print "freeze"
        ...
        #laneposNoise = np.random.normal(0, 1., cars.shape[0])
        #cars['lanepos'] = cars['lanepos'] + laneposNoise
        ...
        for carID in carsToUpdate: # have to redo this to reset all state variables
            ...
            printString += " TTNVc "+str(Qstates[carID]['Time to Crossing Car'])
            printString += " TTNVa "+str(Qstates[carID]['Time to Ahead Car'])
            #printString += " TTNV "+str(finalformat.iloc[7])
            #printString += " behind "+str(Qstates[carID]['Time to Behind Car'])
            printString += " DTACa "+str(Qstates[carID]['Distance to Ahead Car'])
            printString += " DTACc "+str(Qstates[carID]['Distance to Crossing Car'])
...
#                                    differenceRewards[carID])
        ...
        for carID in np.where(carsThatAct)[0]:
            ...
            laneChange, speedChange, turn, zeroAction = agents[carID].Action_training(QTable, trainingTable, None)
...
#                    err += Sim.moveVehicle(str(carID), newLane, newPos) 
                ...
                if grade=='i' or (grade=='o' and car['lanepos']>5.):
                    ...
                    err += Sim.moveVehicleAlong(str(carID), distance, ':')
            #if grade == 'i':
            #    cars.loc[carID,'priority'] = 1# int((route<3) == priorityRoads)
            
        ttime += 0.1
            
    # include vehicles that end in a freeze
    if ttime >= maxTime:
        frozenCars = (cars['status']==0)|(cars['status']==-1)
        WrongPath = WrongPath + sum(frozenCars)
        CarTime = CarTime + sum(maxTime - cars.loc[frozenCars,'time'])
...
if trainingTable:
    np.save('qtable_light_0911.npy',QTable)
    np.save('qcount_light_0911.npy',Qcount)
...
statsSaveName = 'test_4waynoise.csv'#leftturn_train_'+str(iteration)+'.csv'
