...
#        self.distance_to_car_of_interest =  DistancetoToCarOfInterest
    ...
    def Action_training(self, Qtable, trainingOn, randomstate):
        ...
        if trainingOn:
            choiceProbabilities = [.98, .02]
        else:
            choiceProbabilities = [.999, 0.001]
...
#            self.LaneChange = np.array([0,1,2])
        ...
        # decide how to pick actions from table
        if trainingOn:    
...
#                    raise Exception("agent line 198, shouldn't be here")#ActionpInd =-1
                ActionpInd = getActionFromTable(Qtable[:,:9], trainingOn)
...
#                    raise Exception("agent line 247, shouldn't be here")#ActionpInd =-1
                ActionpInd = getActionFromTable(Qtable[:,9:18], trainingOn) + 9
                ...
                raise Exception("agent line 265, shouldn't be here")#self.LaneChange = 0
            if self.FasterTNV ==0 and self.DTAC <2:# and trainingOn == False:
                self.ChangeSpeed = 1
            if self.Location == 1 and self.Priority == 0:
                self.ChangeSpeed = 1
...
#                    raise Exception("agent line 283, shouldn't be here")#ActionpInd =-1
                ActionpInd = getActionFromTable(Qtable[:,18:], trainingOn) + 18
